...
def update_dataset(ix):
    ct_scans_list = [
        process_scan(path + "/CT.nii.gz") for path in scan_paths[10 * ix : 10 * ix + 10]
    ]
    pet_scans_list = [
    ...
    ct_scans_list = [x for x in ct_scans_list if x is not None]
    pet_scans_list = [x for x in pet_scans_list if x is not None]
    mask_scans_list = [x for x in mask_scans_list if x is not None]

    ct_scans = np.array(ct_scans_list)
    pet_scans = np.array(pet_scans_list)
    mask_scans = np.array(mask_scans_list)
    ...
    ct_dataset = (
        ct_loader.map(train_preprocessing)  # .shuffle(len(x_train))
        .batch(batch_size)
        .prefetch(4)
    )
    pet_dataset = (
        pet_loader.map(train_preprocessing)  # .shuffle(len(x_train))
        .batch(batch_size)
        .prefetch(2)
    )
    mask_dataset = (
        mask_loader.map(train_preprocessing)  # .shuffle(len(x_train))
        .batch(batch_size)
        .prefetch(2)
    )
...
def plot_slices(num_rows, num_columns, width, height, data_ct, data_pet, data_mask, i):
    ...
    for j in range(len(list(s))):
        ...
        plt.imsave(
            "combined/combined" + str(i) + "_" + str(j) + "_" + hasmask + ".png",
            np.rot90(comb),
        )
        plt.imsave(
            "masks/mask" + str(i) + "_" + str(j) + "_" + hasmask + ".png",
            np.rot90(sagital_mask),
        )
